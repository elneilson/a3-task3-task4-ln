...
```
The means of these two samples (annual and control) are quite similar, with only a 5% percent difference between them. The standard deviation of the annual sample (0.97) and control sample (0.87) are also fairly similar. This tells me that it is somewhat likely that they could have been drawn from populations with the same mean.

I expect the difference in mean urchin size between species to be within this confidence interval range: -0.1386165 to 0.5203369, for 95% of the samples that are taken.

The p-value is a probability that tells me that if these samples are drawn from populations with the same mean size, then there is approximately a 25% chance that I could have taken two random samples from those populations and found samples at least as different as the samples observed here.

This tells me that it is possible I could have retrieved my samples from populations with the same mean, but it is uncertain.
